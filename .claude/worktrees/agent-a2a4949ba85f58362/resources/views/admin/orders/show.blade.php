...
@section('content')
    ...
    <div class="flex gap-6 items-start">
        ...
        <div class="flex-1 min-w-0 space-y-6">
            ...
            {{-- ──────────────────────────────────── --}}
            <x-card title="{{ __('admin.orders.items') }}">
                <x-slot:actions>
                        ...
                        <div class="border border-gray-200 rounded-xl mb-3 last:mb-0 overflow-hidden">
                            ...
                            <div class="sub-order-body">
                                    ...
                                    <table class="min-w-full text-sm">
                                        <thead class="bg-gray-50/50 border-b border-gray-100">
                                            <tr>
                                                ...
                                                <th class="px-4 py-2.5 text-start text-xs font-semibold text-gray-500 uppercase">{{ __('admin.sku') }}
                                                </th>
                                                <th
                                ...
                                <div
                                    ...
                                    <div>
                                        <span class="font-medium text-gray-700">{{ __('admin.carriers') }}:</span>
                                        {{ $subOrder->carrier->name ?? '—' }}
                                    </div>
                                    <div>
                                        <span class="font-medium text-gray-700">{{ __('admin.tracking') }}:</span>
                                    ...
                                    <div>
                                        <span class="font-medium text-gray-700">{{ __('admin.orders.delivered_at') }}:</span>
                    ...
                    : '—' }}
                                ...
                                <div class="px-4 py-3 border-t border-gray-100 space-y-1 text-xs">
                                    <div class="flex items-center justify-between">
                                        <span class="text-gray-500">{{ __('admin.commission') }}</span>
                                        <span class="text-danger-600">−{{ $fmt($subOrder->platform_commission) }}</span>
                                    </div>
                                    @if($subOrder->gateway_fee > 0)
                                        <div class="flex items-center justify-between">
                                            <span class="text-gray-500">{{ __('admin.orders.payment_processing_fee') }} ({{ __('admin.orders.gateway_fee_vendor_borne') }})</span>
                                    ...
                                    <div class="flex items-center justify-between font-medium text-gray-700">
                                        <span>{{ __('admin.orders.net_vendor_payout') }}</span>
                ...
                @empty
                    <p class="text-sm text-gray-400 italic py-4 text-center">{{ __('admin.no_results') }}</p>
                @endforelse
            </x-card>
            ...
            {{-- Shipping Assignment --}}
            {{-- ──────────────────────────────────── --}}
            <x-card title="{{ __('admin.orders.assign_shipping') }}">
                @forelse($order->subOrders as $subOrder)
                        ...
                        <div>
                            ...
                            @if($subOrder->shippingMethod)
                                <p class="text-sm text-gray-600 shipping-assignment-summary">
                                    {{ __('admin.assign') }}: <span class="font-medium text-gray-900">{{ $subOrder->shippingMethod->name }}</span>
                                    @if($subOrder->carrier)
                                        via <span class="font-medium text-gray-900">{{ $subOrder->carrier->name }}</span>
                                    @endif
                                </p>
                            @else
                                <p class="text-sm font-medium text-amber-600 shipping-assignment-summary">{{ __('admin.orders.select_shipping_method') }}</p>
                            @endif
                        </div>
                        <button type="button"
                            ...
                            @endif>
                            {{ $subOrder->shippingMethod ? 'Reassign' : __('admin.orders.assign_shipping') }}
                        </button>
                    </div>
                @empty
                    <p class="text-sm text-gray-400 italic py-4 text-center">{{ __('admin.no_results') }}</p>
            ...
            <x-card title="{{ __('admin.orders.payment_transactions') }}">
                ...
                @empty
                    <p class="text-sm text-gray-400 italic py-4 text-center">{{ __('admin.no_records') }}</p>
                @endforelse
            </x-card>
            ...
            @if($order->disputes->isNotEmpty())
                <x-card title="{{ __('admin.disputes') }}">
                    @foreach($order->disputes as $dispute)
                        <div class="border border-gray-200 rounded-xl p-4 mb-3 last:mb-0">
                            ...
                            <p class="text-xs font-medium text-gray-500 mb-1">
                                {{ __('admin.reason') }}: {{ $disputeReasonLabels[$dispute->reason] ?? $dispute->reason }}
                            </p>
        ...
        <div class="w-80 flex-shrink-0 space-y-4 sticky top-20">
            ...
            {{-- Order Summary --}}
            <x-card title="{{ __('admin.order_details') }}">
                <div class="space-y-1 text-sm">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500">{{ __('admin.orders.placed_at') }}</span>
                ...
                <div class="border-t border-gray-100 mt-3 pt-3 space-y-1.5 text-sm">
                    <div class="flex justify-between text-gray-600">
                        <span>{{ __('admin.subtotal') }}</span><span>{{ $fmt($order->subtotal) }}</span>
                    </div>
                    @if($order->discount > 0)
                        <div class="flex justify-between text-success-600">
                            <span>{{ __('admin.discount') }}</span><span>−{{ $fmt($order->discount) }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between text-gray-600">
                        <span>{{ __('admin.shipping') }}</span><span>{{ $fmt($order->shipping) }}</span>
                    </div>
                    <div class="flex justify-between text-gray-600">
                        <span>{{ __('admin.tax') }}</span><span>{{ $fmt($order->tax) }}</span>
                    </div>
                    @if($order->cod_fee > 0)
                        <div class="flex justify-between text-gray-600">
                            <span>COD {{ __('admin.fee') }}</span><span>{{ $fmt($order->cod_fee) }}</span>
                        </div>
                    @endif
                    @if($order->coupon_code_used)
                        <div class="flex justify-between text-gray-500 text-xs">
                            <span>{{ __('admin.nav.coupons') }}</span><span class="font-mono">{{ $order->coupon_code_used }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between font-semibold text-gray-900 text-base pt-1 border-t border-gray-100">
                        <span>{{ __('admin.total') }}</span><span>{{ $fmt($order->total) }}</span>
                    </div>
                </div>
                ...
                @if($order->payment_method === 'cod')
                    <div class="border-t border-gray-100 mt-3 pt-3 space-y-1 text-xs">
                        <p class="text-gray-500 font-medium uppercase tracking-wide">{{ __('admin.orders.cod_status') }}</p>
            ...
            <x-card title="{{ __('admin.orders.customer') }}">
                ...
                @if(!empty($addr))
                    <div class="mt-3 pt-3 border-t border-gray-100">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">{{ __('admin.orders.shipping_address') }}</p>
            ...
            <x-card title="{{ __('admin.dashboard.risk_assessment') }}">
                <div class="flex items-center gap-4 mb-3">
                    @if($riskScore !== null)
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">{{ __('admin.orders.risk_score') }}</p>
                        </div>
                    @else
                        <p class="text-sm text-gray-400 italic">{{ __('admin.not_available') }}</p>
                    @endif
                </div>

                <div class="space-y-2 text-xs">
                    <div class="flex items-start gap-2">
                        <span class="text-gray-400 flex-shrink-0 w-20">{{ __('admin.activity_log_section.ip_address') }}</span>
                ...
                @if($fraudFlags->isNotEmpty())
                    <div class="mt-3 pt-3 border-t border-gray-100">
                        <p class="text-xs font-semibold text-danger-600 uppercase tracking-wider mb-2">{{ __('admin.orders.risk_flags') }}</p>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    @if(empty($allowedNextStatuses))
                        <p class="text-sm text-gray-400 italic">{{ __('admin.no_results') }}</p>
                    @else
                ...
                <div>
                    <label class="form-label" for="order-status-reason">{{ __('admin.reason') }} / {{ __('admin.notes') }}</label>
                    <textarea id="order-status-reason" name="reason" rows="3" class="form-textarea w-full"
                        placeholder="{{ __('admin.notes') }}…"></textarea>
                </div>
            </div>
            <x-slot:footer>
                <button type="button" data-modal-close class="btn btn-ghost">{{ __('admin.cancel') }}</button>
        ...
        <form id="refund-form">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="space-y-2 mt-1">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm">{{ __('admin.all') }} — <strong>{{ $fmt($order->total) }}</strong></span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="refund_type" value="shipping_only" class="text-primary-600">
                            <span class="text-sm">{{ __('admin.shipping') }} only — <strong>{{ $fmt($order->shipping) }}</strong></span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="refund_type" value="partial" class="text-primary-600">
                            <span class="text-sm">{{ __('admin.finance.refund_amount') }}</span>
                        </label>
                    </div>
                </div>
                ...
                <div id="partial-amount-field" class="hidden">
                    <label class="form-label">{{ __('admin.amount') }} ({{ $currency }})</label>
                ...
                {{-- Sub-order (optional) --}}
                <div>
                    <label class="form-label">{{ __('admin.orders.sub_order') }} ({{ __('admin.optional') }})</label>
                ...
                {{-- Reason --}}
                <div>
                    <label class="form-label">{{ __('admin.reason') }} <span class="text-danger-500">*</span></label>
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm text-gray-700">{{ __('admin.orders.process_refund') }}</span>
                </label>
            </div>
    ...
    {{-- 3. Force Cancel --}}
    <x-modal id="force-cancel-modal" title="{{ __('admin.orders.force_cancel') }}" size="sm">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="reason" rows="3" class="form-textarea w-full"
                        placeholder="{{ __('admin.reason') }}…"></textarea>
                </div>
                ...
                <label class="flex items-center gap-2 cursor-pointer" id="force-override-toggle">
                    ...
                    <span class="text-sm text-gray-700">{{ __('admin.orders.force_cancel') }}</span>
                </label>
            </div>
    ...
    <x-modal id="dispute-modal" title="{{ __('admin.orders.escalate_dispute') }}" size="md">
        <form id="dispute-form">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="form-label">{{ __('admin.description') }} <span class="text-danger-500">*</span></label>
    ...
    {{-- 5. Flag Fraud --}}
    <x-modal id="fraud-modal" title="{{ __('admin.orders.flag_fraud') }}" size="sm">
        <form id="fraud-form">
            @csrf
            <div class="space-y-4">
                ...
                <div>
                    <label class="form-label">{{ __('admin.reason') }} <span class="text-danger-500">*</span></label>
                    <textarea name="reason" rows="3" class="form-textarea w-full"
                        placeholder="{{ __('admin.orders.fraud_reason') }}…"></textarea>
                </div>
            </div>
    ...
    <x-modal id="shipping-assign-modal" title="{{ __('admin.orders.assign_shipping') }}" size="lg">
        <div id="shipping-assign-zone-warning"
            ...
            {{ __('admin.orders.no_disputes') }}
        </div>
        <div id="shipping-assign-loading" class="text-sm text-gray-500 py-8 text-center">{{ __('admin.loading') }}</div>
        ...
        <x-slot:footer>
            <button type="button" data-modal-close class="btn btn-ghost">{{ __('admin.cancel') }}</button>
            <button type="button" id="shipping-assign-confirm" class="btn btn-primary" disabled>{{ __('admin.confirm') }}</button>
        </x-slot:footer>
    </x-modal>
