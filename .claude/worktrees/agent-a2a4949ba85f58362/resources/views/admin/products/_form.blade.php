    ...
    <div class="flex gap-6 items-start">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="bg-white rounded-t-xl border border-gray-200 overflow-hidden">
                <nav class="flex overflow-x-auto border-b border-gray-100" aria-label="Product form tabs">
                    @foreach([
                        ['id' => 'basic',     'label' => __('admin.products.basic_info'),  'icon' => 'information-circle'],
                        ['id' => 'content',   'label' => __('admin.products.content'),     'icon' => 'document-text'],
                        ['id' => 'variants',  'label' => __('admin.products.variants'),    'icon' => 'cube'],
                        ['id' => 'images',    'label' => __('admin.products.images_tab'),  'icon' => 'photo'],
                        ['id' => 'countries', 'label' => __('admin.products.countries'),   'icon' => 'globe-alt'],
                        ['id' => 'seo',       'label' => __('admin.products.seo_tab'),     'icon' => 'magnifying-glass'],
                    ] as $tab)
                    <button
                        type="button"
            ...
            >
                <div class="grid grid-cols-2 gap-4">
                    <x-form.input
                        name="name_en"
                        :label="__('admin.products.product_name_en')"
                        :value="$val('name_en')"
                        required
                        maxlength="255"
                    ...
                    <x-form.input
                        name="name_ar"
                        :label="__('admin.products.product_name_ar')"
                        :value="$val('name_ar')"
                        required
                        maxlength="255"
                        dir="rtl"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="form-label">{{ __('admin.products.short_desc_en') }}</label>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="form-label" dir="rtl">{{ __('admin.products.short_desc_ar') }}</label>
                    ...
                    <div class="overflow-x-auto rounded-xl border border-gray-200">
                        <table class="min-w-full text-sm" id="variants-table">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    <th class="px-4 py-3 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.products.variants') }}</th>
                                    <th class="px-4 py-3 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider w-32">SKU</th>
                                    ...
                                    <th class="px-4 py-3 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider w-28">Weight (g)</th>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 border-b border-gray-100">
                            <tr>
                                <th class="px-6 py-3 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.country') }}</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-28">{{ __('admin.products.available') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.products.name_override') }}</th>
                                <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-32">{{ __('admin.products.cert_required') }}</th>
                            </tr>
                        </thead>
            ...
            >
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="seo_title" class="form-label">{{ __('admin.products.seo_title') }}</label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="seo_description" class="form-label">{{ __('admin.products.seo_description') }}</label>
                ...
                <x-form.slug-input
                    name="slug"
                    :label="__('admin.products.url_slug')"
                ...
                <div class="rounded-xl border border-gray-200 p-4 bg-gray-50">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{{ __('admin.products.search_preview') }}</p>
        ...
        <div class="w-72 flex-shrink-0 sticky top-20 space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
                <h3 class="text-sm font-semibold text-gray-800">{{ __('admin.products.product_status') }}</h3>
                <x-form.select
                    name="status"
                    label=""
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-1">
                <h3 class="text-sm font-semibold text-gray-800 mb-3">{{ __('admin.products.classification') }}</h3>
                ...
                <label class="flex items-center gap-3 py-2 cursor-pointer select-none w-full group">
                    ...
                    <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ __('admin.products.has_variants') }}</span>
                </label>

                <x-form.toggle name="is_featured"         :label="__('admin.products.featured')" :value="$bool('is_featured')" />
                <x-form.toggle name="requires_brand_auth" :label="__('admin.products.requires_brand_auth')"  :value="$bool('requires_brand_auth')" />
                <x-form.toggle name="is_hazardous"        :label="__('admin.products.hazardous')"       :value="$bool('is_hazardous')" />
                ...
                <label class="flex items-center gap-3 py-2 cursor-pointer select-none w-full group">
                    ...
                    <span class="text-sm text-gray-700 group-hover:text-gray-900">{{ __('admin.products.age_restricted') }}</span>
                </label>

                <div x-show="isAgeRestricted" x-cloak class="mt-1 pl-14">
                    <x-form.input
                        name="min_age"
                        :label="__('admin.products.min_age')"
                        type="number"
                        :value="$val('min_age')"
                        min="1" max="99"
                    />
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-2">
                ...
                <button type="submit" id="submit-btn"
                    ...
                    {{ $isEdit ? __('common.save') : __('admin.products.add_product') }}
                </button>
