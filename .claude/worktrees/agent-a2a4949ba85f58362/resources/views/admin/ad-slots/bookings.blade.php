...
@section('title', __('admin.ad_campaigns.bookings_for') . $adSlot->name)
...
@section('content')
    ...
    <div class="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                ...
                <span>/</span>
                <span class="text-gray-800 font-medium">{{ __('admin.ad_campaigns.bookings') }}</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.ad_campaigns.bookings') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">
                {{ __('admin.ad_campaigns.slot_label') }}: <strong>{{ $adSlot->name }}</strong>
                <span class="font-mono ml-2 text-xs text-gray-400">{{ $adSlot->slot_code }}</span>
            </p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.ad-slots.edit', $adSlot->id) }}" class="btn btn-secondary btn-sm">{{ __('admin.ad_campaigns.edit_slot') }}</a>
            <a href="{{ route('admin.ad-slots.index') }}" class="btn btn-secondary btn-sm">{{ __('admin.ad_campaigns.all_slots') }}</a>
        </div>
    </div>
    ...
    <x-card class="mb-6">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
                <dt class="text-xs text-gray-400 uppercase font-medium mb-0.5">{{ __('admin.ad_campaigns.pricing_model') }}</dt>
                <dd class="font-semibold uppercase">{{ $adSlot->pricing_model ?? '—' }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 uppercase font-medium mb-0.5">{{ __('admin.ad_campaigns.base_rate') }}</dt>
                <dd class="font-semibold">${{ number_format(($adSlot->base_rate_cents ?? 0) / 100, 2) }}</dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 uppercase font-medium mb-0.5">{{ __('admin.status') }}</dt>
                <dd>
                    @if($adSlot->is_available)
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-success-100 text-success-700">{{ __('admin.ad_campaigns.available') }}</span>
                    @else
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{{ __('admin.ad_campaigns.unavailable') }}</span>
                    @endif
                </dd>
            </div>
            <div>
                <dt class="text-xs text-gray-400 uppercase font-medium mb-0.5">{{ __('admin.ad_campaigns.country') }}</dt>
    ...
    {{-- Bookings table --}}
    <x-card title="{{ __('admin.ad_campaigns.bookings') }}">
        @if($bookings->isEmpty())
            <p class="text-sm text-gray-400 py-6 text-center">{{ __('admin.ad_campaigns.no_bookings_found') }}</p>
        @else
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="py-2">
                                    <a href="{{ route('admin.paid-ad-bookings.show', $booking->id) }}"
                                        class="btn btn-xs btn-secondary">{{ __('common.view') }}</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
