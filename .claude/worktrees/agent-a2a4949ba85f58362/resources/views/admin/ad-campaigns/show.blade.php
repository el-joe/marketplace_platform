...
@section('title', __('admin.ad_campaigns.campaign_title_prefix') . $campaign->name)
...
@section('content')
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @elseif($campaign->status === 'active')
                <button type="button"
                    ...
                    data-name="{{ e($campaign->name) }}">
                    {{ __('admin.ad_campaigns.pause_campaign') }}
                </button>
            @elseif($campaign->status === 'paused')
                <button type="button"
                    ...
                    data-name="{{ e($campaign->name) }}">
                    {{ __('admin.ad_campaigns.resume_campaign') }}
                </button>
            @endif
            <a href="{{ route('admin.ad-campaigns.index') }}" class="btn btn-secondary">{{ __('common.back') }}</a>
        </div>
    </div>
    ...
    @if($campaign->status === 'rejected' && $campaign->rejection_reason)
        <div class="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
            <strong>{{ __('admin.ad_campaigns.rejection_reason_label') }}</strong> {{ $campaign->rejection_reason }}
        </div>
    @endif
    ...
    <div
        ...
        <div class="flex border-b border-gray-200 gap-1 overflow-x-auto">
            @foreach([
                ...
                'products'    => __('admin.ad_campaigns.tab_products') . ' (' . $campaign->products->count() . ')',
                'keywords'    => __('admin.ad_campaigns.tab_keywords') . ' (' . $campaign->keywords->count() . ')',
                'categories'  => __('admin.ad_campaigns.tab_categories') . ' (' . $campaign->categoryTargets->count() . ')',
                'performance' => __('admin.ad_campaigns.tab_performance'),
                'fraud'       => __('admin.ad_campaigns.tab_fraud') . ' (' . $campaign->fraudPatterns->count() . ')',
            ] as $tab => $label)
                <button
                    type="button"
                    @click="activeTab = '{{ $tab }}'"
        ...
        <div x-show="activeTab === 'overview'" x-transition>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <x-card title="{{ __('admin.ad_campaigns.campaign_details') }}">
                        <dl class="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 text-sm">
                            ...
                            <div>
                                <dt class="text-gray-500 text-xs uppercase font-medium mb-0.5">{{ __('admin.status') }}</dt>
                                <dd>
                                    @php
                            ...
                            <div>
                                <dt class="text-gray-500 text-xs uppercase font-medium mb-0.5">{{ __('admin.ad_campaigns.targeting') }}</dt>
                    ...
                    <x-card title="{{ __('admin.ad_campaigns.performance_last_7') }}">
                ...
                <div class="space-y-5">
                    <x-card title="{{ __('admin.ad_campaigns.quality_score') }}">
                        @if($qualityScore)
                            @php
                                $scores = [
                                    __('admin.ad_campaigns.score_overall')   => $qualityScore->score,
                                    __('admin.ad_campaigns.score_ctr')       => $qualityScore->ctr_score,
                                    __('admin.ad_campaigns.score_relevance') => $qualityScore->relevance_score,
                                    __('admin.ad_campaigns.score_landing')   => $qualityScore->landing_score,
                                    __('admin.ad_campaigns.score_vendor')    => $qualityScore->vendor_score,
                                ];
                            @endphp
                            <div class="space-y-3">
        ...
        <div x-show="activeTab === 'products'" x-transition>
            <x-card title="{{ __('admin.ad_campaigns.promoted_products') }}">
                @if($campaign->products->isEmpty())
                    <p class="text-sm text-gray-400 py-6 text-center">{{ __('admin.ad_campaigns.no_products') }}</p>
                @else
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-100 text-start">
                                    ...
                                    <th class="py-2 pr-4 text-xs font-medium text-gray-500 uppercase">{{ __('common.active') }}</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-2">
                                            @if($product->is_active)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-success-100 text-success-700">{{ __('common.active') }}</span>
                                            @else
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-100 text-start">
                                    ...
                                    <th class="py-2 text-xs font-medium text-gray-500 uppercase">{{ __('common.active') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
        ...
        <div x-show="activeTab === 'performance'" x-transition>
            ...
            {{-- Chart --}}
            <x-card title="{{ __('admin.ad_campaigns.impressions_clicks_30') }}" class="mb-5">
            ...
            <x-card title="{{ __('admin.ad_campaigns.daily_breakdown') }}">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 text-start">
                                <th class="py-2 pr-4 text-xs font-medium text-gray-500 uppercase">{{ __('admin.ad_campaigns.date') }}</th>
                                        ...
                                        <td class="py-2">
                                            @if(!$fp->is_blocked)
                                                <button type="button"
                                                    ...
                                                    {{ __('admin.ad_campaigns.block_ip') }}
                                                </button>
                                            @else
    ...
    {{-- ─── Shared Modals ──────────────────────────────────────────────────────── --}}
    <x-modal id="approve-modal" title="{{ __('admin.ad_campaigns.approve_campaign') }}" size="sm">
        <p class="text-sm text-gray-600">{{ __('admin.ad_campaigns.approve_confirm_prefix') }} <strong id="approve-campaign-name"></strong>{{ __('admin.ad_campaigns.approve_confirm_suffix') }}</p>
        <div class="flex justify-end gap-3 mt-5">
            <button type="button" class="btn btn-secondary" onclick="$('#approve-modal').modal('close')">{{ __('common.cancel') }}</button>
            <button type="button" id="confirm-approve-btn" class="btn btn-success">{{ __('admin.approve') }}</button>
        </div>
    </x-modal>

    <x-modal id="reject-modal" title="{{ __('admin.ad_campaigns.reject_campaign') }}" size="md">
        <p class="text-sm text-gray-600 mb-3">{{ __('admin.ad_campaigns.reject_confirm_prefix') }} <strong id="reject-campaign-name"></strong>.</p>
        ...
        <textarea id="reject-reason-input" rows="3" class="form-input w-full text-sm" placeholder="{{ __('admin.ad_campaigns.rejection_reason_placeholder') }}"></textarea>
        ...
        <div class="flex justify-end gap-3 mt-5">
            ...
            <button type="button" id="confirm-reject-btn" class="btn btn-danger">{{ __('admin.reject') }}</button>
        </div>
    </x-modal>

    <x-modal id="pause-modal" title="{{ __('admin.ad_campaigns.pause_campaign') }}" size="sm">
        <p class="text-sm text-gray-600">{{ __('admin.ad_campaigns.pause_confirm_prefix') }} <strong id="pause-campaign-name"></strong>{{ __('admin.ad_campaigns.pause_confirm_suffix') }}</p>
        <div class="flex justify-end gap-3 mt-5">
    ...
    <x-modal id="resume-modal" title="{{ __('admin.ad_campaigns.resume_campaign') }}" size="sm">
        <p class="text-sm text-gray-600">{{ __('admin.ad_campaigns.resume_confirm_prefix') }} <strong id="resume-campaign-name"></strong>{{ __('admin.ad_campaigns.resume_confirm_suffix') }}</p>
        <div class="flex justify-end gap-3 mt-5">
    ...
    <x-modal id="block-ip-modal" title="{{ __('admin.ad_campaigns.block_ip_address') }}" size="md">
        <p class="text-sm text-gray-600 mb-3">{{ __('admin.ad_campaigns.block_ip_prefix') }} <strong id="block-ip-address" class="font-mono"></strong>?</p>
        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.ad_campaigns.block_reason') }}</label>
        <input type="text" id="block-reason-input" class="form-input w-full text-sm" placeholder="{{ __('admin.ad_campaigns.block_reason_short_placeholder') }}">
        <div class="flex justify-end gap-3 mt-5">
