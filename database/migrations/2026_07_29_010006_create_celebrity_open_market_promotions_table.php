...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('celebrity_open_market_promotions', function (Blueprint $table) {
            ...
            $table->char('promoter_marketer_id', 36)->index();
            // The celebrity store product being promoted
            $table->char('celebrity_store_product_id', 36)->index();
            // The celebrity who owns the product (denormalized for query efficiency)
            $table->char('owner_marketer_id', 36)->index();
            ...
            $table->foreign('promoter_marketer_id')->references('id')->on('marketers')->onDelete('restrict');
            $table->foreign('celebrity_store_product_id')
                  ->references('id')->on('celebrity_store_products')->onDelete('restrict');
            $table->foreign('owner_marketer_id')->references('id')->on('marketers')->onDelete('restrict');
        });
    }
