...
@section('content')
    ...
    <div class="mx-4 mb-5 bg-gradient-to-br from-slate-700 to-slate-900 rounded-2xl p-5 border border-slate-600">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Available Balance</p>
        ...
        @if($wallet->pending_balance_cents > 0)
            <p class="text-xs text-yellow-400 mt-2">{{ number_format($wallet->pending_balance_cents / 100, 2) }} {{ $wallet->currency }} pending withdrawal</p>
        @endif
        @if($wallet->is_frozen)
            <div class="mt-3 inline-flex items-center gap-1.5 bg-red-500/20 text-red-400 text-xs font-medium px-3 py-1 rounded-full border border-red-500/30">Wallet Frozen</div>
        @endif
    </div>
    ...
    <div class="mx-4 mb-5 d-card">
        <h2 class="text-sm font-bold text-slate-300 mb-4">Request Withdrawal</h2>
        <form method="POST" action="{{ route('delivery.wallet.withdraw') }}" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs text-slate-400 mb-1">Amount ({{ $wallet->currency }})</label>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">Bank IBAN</label>
    ...
    <h2 class="mx-4 text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Withdrawal Requests</h2>
    ...
    {{-- Transaction History --}}
    <h2 class="mx-4 text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Transactions</h2>
    <div class="mx-4 mb-5 space-y-2">
        ...
        @empty
            <div class="d-card text-center py-8 text-sm text-slate-500">No transactions yet.</div>
        @endforelse
