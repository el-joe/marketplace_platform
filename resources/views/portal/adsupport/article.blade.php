...
@section('title', $article->localizedTitle() . ' | ' . ($isAr ? 'أعلن بذكاء، وانمُ أسرع' : 'Advertise smarter, grow faster'))
    ...
    <div class="justify-between flex">
        <div class="relative z-3 w-full lg:max-w-[640px]">
                    ...
                    <div class="mb-7 ml-0 text-md lg:hidden" x-data="{ tocOpen: false }">
                        <div class="max-h-[calc(100vh-96px)] overflow-y-auto rounded-2xl border border-solid border-[#e6e6e6] text-black">
                            <div @click="tocOpen = !tocOpen" class="flex cursor-pointer flex-row justify-between border-b border-solid border-[#e6e6e6] px-4 py-2">
                                <div>{{ $isAr ? 'محتويات الصفحة' : 'Table of contents' }}</div>
                ...
                <div class="article_body">
                    ...
                    @if($relatedArticles->isNotEmpty())
                        <section class="my-6">
                            ...
                            <div class="mb-3 text-xl font-bold">{{ $isAr ? 'مقالات ذات صلة' : 'Related Articles' }}</div>
            ...
            <fieldset class="m-0 mt-6 rounded-[10px] border-0 sm:mt-8" x-data="{ picked: null }">
                <legend class="float-start w-full">{{ $isAr ? 'هل أجاب هذا المقال على سؤالك؟' : 'Did this answer your question?' }}</legend>
                <div class="mt-4 flex gap-2">
                    <button type="button" @click="picked = 'disappointed'" :class="picked === 'disappointed' ? 'bg-black/10' : ''" class="rounded-full border border-solid border-[#e6e6e6] px-3 py-2" aria-label="{{ $isAr ? 'رد فعل مستاء' : 'Disappointed Reaction' }}"><span aria-hidden="true">😞</span></button>
                    <button type="button" @click="picked = 'neutral'" :class="picked === 'neutral' ? 'bg-black/10' : ''" class="rounded-full border border-solid border-[#e6e6e6] px-3 py-2" aria-label="{{ $isAr ? 'رد فعل محايد' : 'Neutral Reaction' }}"><span aria-hidden="true">😐</span></button>
                    <button type="button" @click="picked = 'smiley'" :class="picked === 'smiley' ? 'bg-black/10' : ''" class="rounded-full border border-solid border-[#e6e6e6] px-3 py-2" aria-label="{{ $isAr ? 'رد فعل مبتسم' : 'Smiley Reaction' }}"><span aria-hidden="true">😊</span></button>
                </div>
            </fieldset>
        </div>
        ...
        @if(!empty($toc))
            <div class="w-61 sticky top-8 ml-7 max-w-61 self-start max-lg:hidden mt-16">
                <div class="max-h-[calc(100vh-96px)] overflow-y-auto rounded-2xl text-black">
                    <div class="my-2">{{ $isAr ? 'محتويات الصفحة' : 'Table of contents' }}</div>
                    <div class="my-2">
                        @foreach($toc as $item)
