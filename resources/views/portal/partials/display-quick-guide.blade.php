    ...
    <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pb-12 lg:pb-16">
        ...
        <div class="relative rounded-2xl overflow-hidden bg-gray-100 aspect-video max-w-[960px] mx-auto shadow-sm"
            ...
            <button x-show="playing" x-cloak @click.stop="seek(-10)"
                ...
                <img src="https://advertise.noon.com/icons/rewind10.svg" alt="{{ $isAr ? 'إرجاع 10 ثوانٍ' : 'Skip backward 10 seconds' }}" class="w-full h-full">
            </button>

            {{-- Center play / pause --}}
            <button @click="toggle" class="absolute inset-0 flex items-center justify-center" aria-label="{{ $isAr ? 'تشغيل / إيقاف' : 'Play / Pause' }}">
                <img x-show="!playing" src="https://advertise.noon.com/icons/play.png" alt="{{ $isAr ? 'تشغيل' : 'Play' }}"
            ...
            <button x-show="playing" x-cloak @click.stop="seek(10)"
                    class="absolute {{ $isAr ? 'start-1/2 ms-16 sm:ms-24' : 'end-1/2 me-16 sm:me-24' }} top-1/2 -translate-y-1/2
                           w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center opacity-90 hover:opacity-100 transition-opacity">
                <img src="https://advertise.noon.com/icons/forward10.svg" alt="{{ $isAr ? 'تقديم 10 ثوانٍ' : 'Skip forward 10 seconds' }}" class="w-full h-full">
            </button>
            ...
            <div class="absolute inset-x-0 bottom-0 flex items-center gap-3 px-3 sm:px-4 py-3 bg-gradient-to-t from-black/60 to-transparent">
                <button @click.stop="toggleMute" class="text-white shrink-0" aria-label="{{ $isAr ? 'كتم الصوت' : 'Mute' }}">
