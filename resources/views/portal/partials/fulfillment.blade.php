    ...
    <div class="rounded-2xl overflow-hidden bg-[#1c1c1c] grid md:grid-cols-[1.5fr_2fr]">
        <div class="relative aspect-[4/3] md:aspect-auto">
            <img src="https://f.nooncdn.com/s/app/pr-comms/sell-with-us/03-fbn.jpg"
                 alt="{{ $isAr ? 'الشحن والتوصيل' : 'Shipping & fulfilment' }}"
                 class="absolute inset-0 w-full h-full object-cover">
        </div>
        <div class="pt-8 px-6 pb-8 md:pt-10 md:px-10 md:pb-10">
            <h2 class="text-yellow-400 font-black text-xl lg:text-2xl">{{ $isAr ? 'الشحن والتوصيل' : 'Shipping & fulfilment' }}</h2>
            <h3 class="text-white font-black text-lg lg:text-xl mt-1 mb-4">
                {{ $isAr ? 'خيارات تنفيذ مرنة تناسبك' : 'Flexible fulfilment options that fit you' }}
            </h3>
            <p class="text-gray-300 text-[15px] font-medium mb-6">
                {{ $isAr ? 'اختر النموذج الذي يناسب عملك اليوم، وتوسع بثقة' : 'Choose the model that fits your business today, and scale with confidence' }}
            </p>
            <ul class="space-y-3 mb-8">
                @foreach([
                    [$isAr ? 'التنفيذ من قبل نون - مصمم للسهولة والنمو' : 'Fulfilled by noon - built for ease and growth'],
                    [$isAr ? 'التنفيذ من قبل الشريك - مصمم للتحكم والمرونة' : 'Fulfilled by partner - built for control and flexibility'],
                ] as $item)
