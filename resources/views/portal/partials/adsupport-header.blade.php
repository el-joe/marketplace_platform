        ...
        <div class="flex h-full flex-col items-center">
            ...
            <section class="relative flex w-full flex-col mb-6 pb-6">
                <div class="flex justify-center px-5 pt-6 leading-none sm:px-10">
                    <div class="flex items-center w-full lg:w-[960px]">
                        <div class="mo__body">
                            <a href="{{ $homeUrl }}" class="block" aria-label="{{ $isAr ? 'أعلن بذكاء، وانمُ أسرع' : 'Advertise smarter, grow faster' }}">
                                <img src="https://downloads.intercomcdn.com/i/o/yba8j1xj/654495/fd5bbb8bb174fa85e45ad370b70b/d9702cdb7e2403024b5c1d0d2a3f23e3.png"
                                     height="60" alt="{{ $isAr ? 'أعلن بذكاء، وانمُ أسرع' : 'Advertise smarter, grow faster' }}" class="h-[60px] w-auto">
                            </a>
                        </div>
                        ...
                        <div class="ms-auto flex items-center font-semibold">
                            ...
                            <nav class="hidden items-center md:flex">
                                <a target="_blank" rel="noopener noreferrer" href="{{ route('portal.register') }}"
                                   class="mx-5 text-md no-underline hover:opacity-80 md:mx-3 md:text-base">{{ $isAr ? 'ابدأ الآن' : 'Start now' }}</a>
                                <a href="{{ route('portal.advertise.request', $country) }}"
                                   class="mx-3 text-md no-underline hover:opacity-80 md:text-base">{{ $isAr ? 'تواصل معنا' : 'Contact us' }}</a>
                            </nav>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="mobileOpen" x-cloak x-transition class="fixed right-0 top-0 z-50 h-full w-full md:hidden">
                    <div class="flex h-full w-full justify-end bg-black bg-opacity-30" @click="mobileOpen = false">
                        <div class="flex h-fit w-full flex-col bg-white opacity-100 sm:w-1/2" @click.stop>
                            <button type="button" @click="mobileOpen = false" class="flex items-center self-end border-none bg-transparent pr-6 pt-6" aria-label="{{ $isAr ? 'إغلاق القائمة' : 'Close menu' }}">
                            ...
                            <nav class="flex flex-col pl-4 text-black">
                                <a target="_blank" rel="noopener noreferrer" href="https://admanager.noon.partners/en-ae?utm_source=help_center&utm_medium=header" class="mx-5 mb-5 text-md no-underline hover:opacity-80">{{ $isAr ? 'ابدأ الآن' : 'Start now' }}</a>
                                <a href="{{ route('portal.advertise.request', $country) }}" class="mx-5 mb-5 text-md no-underline hover:opacity-80">{{ $isAr ? 'تواصل معنا' : 'Contact us' }}</a>
                            </nav>
                        </div>
                    </div>
                </div>
            ...
            <section class="relative mx-5 flex h-full w-full flex-col items-center px-5 sm:px-10">
                <div class="flex h-full max-w-full flex-col w-full lg:w-[960px] justify-center">
                    @if($variant === 'home')
                        <h1 class="text-3xl sm:text-4xl mb-6 font-bold text-black text-center">{{ $isAr ? 'مرحباً، كيف يمكننا مساعدتك؟' : 'Hi, how can we help you?' }}</h1>
                    @endif
                        ...
                        <form action="{{ $homeUrl }}" method="GET" autocomplete="off">
                            <div class="flex w-full flex-col items-center">
                                <div class="relative flex w-full sm:w-[640px]">
                                    <label for="search-input" class="sr-only">{{ $isAr ? 'ابحث في المقالات...' : 'Search for articles...' }}</label>
                                    <input id="search-input" type="text" name="q" autocomplete="off" value="{{ request('q') }}"
                                           placeholder="{{ $isAr ? 'ابحث في المقالات...' : 'Search for articles...' }}" aria-label="{{ $isAr ? 'ابحث في المقالات...' : 'Search for articles...' }}"
