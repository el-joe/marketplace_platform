...
@section('title', 'باقات السفر والسياحة')
...
<div class="max-w-7xl mx-auto px-4 py-8 space-y-8">
    ...
    <div class="bg-gradient-to-l from-blue-600 to-blue-800 rounded-3xl p-8 text-white">
        <h1 class="text-3xl font-black mb-2">اكتشف العالم معنا ✈</h1>
        <p class="text-blue-200 text-lg">باقات سياحية مختارة من أفضل مكاتب السفر المعتمدة</p>
    </div>
    ...
    <form method="GET" class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">الدولة</label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">المدينة</label>
                <input type="text" name="city" value="{{ request('city') }}" placeholder="اسم المدينة"
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">أقصى سعر</label>
                <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="مثال: 5000"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 bg-blue-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-blue-500">بحث</button>
                <a href="{{ url()->current() }}" class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">مسح</a>
            </div>
        </div>
    </form>
    ...
    <div class="text-center py-16">
        ...
        <p class="text-gray-500 text-lg">لا توجد باقات متاحة بهذه المواصفات</p>
        <a href="{{ url()->current() }}" class="mt-4 inline-block text-blue-600 hover:underline text-sm">عرض كل الباقات</a>
    </div>
    @else
    <div>
        <p class="text-sm text-gray-500 mb-4">{{ $packages->total() }} باقة متاحة</p>
               ...
               class="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow group">
                ...
                <div class="p-4 space-y-2">
                    ...
                    <div class="flex items-center gap-3 text-xs text-gray-500">
                        ...
                        <span>⏱ {{ $pkg->duration_days }}ي / {{ $pkg->duration_nights }}ل</span>
                    </div>
                    @if($pkg->available_seats !== null)
                    <p class="text-xs text-amber-600">{{ $pkg->seatsRemaining() }} مقعد متاح</p>
                    @endif
                    <div class="pt-2 border-t border-gray-100 flex items-center justify-between">
                        <span class="font-black text-gray-900">{{ $pkg->priceFormatted() }}</span>
                        <span class="text-xs text-gray-400">للفرد</span>
                    </div>
                </div>
            </a>
            @endforeach
