...
<div class="mb-6">
    <h1 class="text-2xl font-black text-gray-900">مرحباً، {{ auth('shipping_supervisor')->user()->name }}</h1>
    <p class="text-sm text-gray-500 mt-0.5">{{ $company->name }} — آخر تحديث: {{ now()->format('H:i') }}</p>
</div>

{{-- Stats --}}
<div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
    @foreach([
        ['label' => 'المناديب الكليون',   'value' => $stats['total_agents'],    'color' => 'gray'],
        ['label' => 'المناديب النشطون',   'value' => $stats['active_agents'],   'color' => 'emerald'],
        ['label' => 'طلبات معلّقة',       'value' => $stats['pending'],         'color' => 'amber'],
        ['label' => 'قيد التنفيذ',        'value' => $stats['in_progress'],     'color' => 'blue'],
        ['label' => 'مسلّم اليوم',        'value' => $stats['delivered_today'], 'color' => 'indigo'],
    ] as $stat)
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="{{ route('carrier.assignments.index') }}" class="text-sm text-indigo-600 hover:underline">عرض الكل</a>
        @endif
    </div>

    @if($recentAssignments->isEmpty())
    <div class="px-6 py-10 text-center text-gray-400 text-sm">لا توجد طلبات حتى الآن.</div>
    @else
