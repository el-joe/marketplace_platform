...
@section('title', 'شحنات غير معينة')

@section('content')

    <div class="mb-6">
        <h1 class="text-2xl font-black text-gray-900">شحنات غير معينة</h1>
        <p class="text-sm text-gray-500 mt-0.5">شحنات لم يُعيَّن لها مندوب توصيل بعد.</p>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        @if($shipments->isEmpty())
            <div class="py-16 text-center text-gray-400 text-sm">لا توجد شحنات غير معينة.</div>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                        <tr>
                            <th class="px-6 py-3 text-right font-semibold">رقم التتبع</th>
                            <th class="px-6 py-3 text-right font-semibold">رقم الطلب</th>
                            <th class="px-6 py-3 text-right font-semibold">المدينة</th>
                            <th class="px-6 py-3 text-right font-semibold">منذ</th>
                            @if(auth('shipping_supervisor')->user()->hasPermission('assign_orders'))
                                <th class="px-6 py-3 text-right font-semibold">تعيين</th>
                            @endif
                        </tr>
                    </thead>
        ...
        <div id="assign-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/40 px-4">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
                <div class="flex items-center justify-between mb-1">
                    <h2 class="text-lg font-bold text-gray-900">تعيين مندوب</h2>
                ...
                @if($agents->isEmpty())
                    <p class="text-sm text-gray-500 text-center py-6">لا يوجد مناديب نشطون.</p>
                @else
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        ...
                                        @if(!$agent->is_available)
                                            <span class="text-amber-500 mr-1">· غير متاح</span>
                                        @endif
                                    </p>
                                </div>
                                @if($agent->rating_avg)
