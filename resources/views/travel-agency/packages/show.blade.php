...
<div class="max-w-4xl space-y-6">
    ...
    <div class="flex items-start justify-between">
        ...
        <div class="flex gap-2">
            ...
            @endif
            <a href="{{ route('travel-agency.packages.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm">← رجوع</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between"><dt class="text-gray-500">الوجهة</dt><dd class="text-gray-900">{{ $package->destination_country }}{{ $package->destination_city ? '، '.$package->destination_city : '' }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">المغادرة</dt><dd class="text-gray-900">{{ $package->departure_date->format('d M Y') }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">العودة</dt><dd class="text-gray-900">{{ $package->return_date->format('d M Y') }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">المدة</dt><dd class="text-gray-900">{{ $package->duration_days }} أيام / {{ $package->duration_nights }} ليالي</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">السعر للفرد</dt><dd class="text-gray-900 font-bold">{{ $package->priceFormatted() }}</dd></div>
                <div class="flex justify-between"><dt class="text-gray-500">المقاعد</dt><dd class="text-gray-900">{{ $package->seats_booked }} محجوزة / {{ $package->available_seats ?? '∞' }} متاح</dd></div>
            </dl>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            ...
            <ul class="space-y-1 text-sm text-gray-700">
                ...
                <li class="flex items-center gap-2"><span class="text-emerald-500">✓</span> {{ $inclusionLabels[$item] ?? $item }}</li>
                @endforeach
            </ul>
            @else
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-4 gap-3">
            ...
            @else
            <video src="{{ $m->url() }}" controls class="rounded-lg h-28 w-full object-cover border border-gray-200"></video>
            @endif
            @endforeach
        </div>
    </div>
