...
@push('scripts')
    @vite('resources/js/travel_agency/team.js')
    <script>
        window.TEAM = {
            csrf: '{{ csrf_token() }}',
            storeUrl: '{{ route('travel-agency.team.store') }}',
            updateRoleUrl: '{{ route('travel-agency.team.update-role', ':id') }}',
            toggleActiveUrl: '{{ route('travel-agency.team.toggle-active', ':id') }}',
            destroyUrl: '{{ route('travel-agency.team.destroy', ':id') }}',
...
@section('content')
    @php
        $roleColors = [
            'travel_agency_owner' => 'bg-purple-100 text-purple-700',
            'travel_agency_manager' => 'bg-blue-100 text-blue-700',
            'travel_agency_staff' => 'bg-gray-100 text-gray-600',
        ];
        $roleLabels = array_merge([
            'travel_agency_owner' => __('travel.team.role_owner'),
            'travel_agency_manager' => __('travel.team.role_manager'),
            'travel_agency_staff' => __('travel.team.role_staff'),
        ], $customRoleLabels ?? []);
    @endphp
    <div class="px-4 py-6 sm:px-6 lg:px-8" x-data="{ showInviteModal: false }">
        ...
        @if (!$admin->isOwner())
            <div class="mb-4 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-700">
                {{ __('travel.team.owner_only_notice') }}
            </div>
        @endif
                        ...
                        <tr class="hover:bg-gray-50 transition-colors" id="member-row-{{ $member->id }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex-shrink-0 h-9 w-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-semibold text-sm select-none">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900">
                                            ...
                                            @if ($member->id === $admin->id)
                                                <span class="text-xs text-gray-400">{{ __('travel.team.you_suffix') }}</span>
                                            @endif
                                        </p>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium member-status-badge
                                    ...
                                    {{ $member->is_active ? __('travel.team.active') : __('travel.team.inactive') }}
                                </span>
                            </td>
                            ...
                            @if ($admin->isOwner())
                                <td class="px-6 py-4 text-left">
                                    @if (!$member->isOwner())
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @unless ($member->id === $admin->id)
                                                <select
                                                    class="select-change-role text-xs font-medium rounded-lg border border-gray-300 px-2 py-1.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                                    data-id="{{ $member->id }}">
                                    ...
                                    : 'border-green-300 text-green-700 hover:bg-green-50' }}"
                                                ...
                                                {{ $member->is_active ? __('travel.team.deactivate') : __('travel.team.activate') }}
                                            </button>
        ...
        <div x-show="showInviteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-xl p-6" @click.stop>
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-base font-semibold text-gray-900">{{ __('travel.team.invite_new_member') }}</h3>
                ...
                <form id="form-invite" novalidate>
                    <div class="space-y-4">
                        {{-- Name --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('travel.team.full_name') }}
                                <span class="text-red-500">*</span></label>
                            <input type="text" name="name" autocomplete="off"
                                class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        </div>
                        ...
                        {{-- Role --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('travel.team.role') }}
                                <span class="text-red-500">*</span></label>
                            <select name="role"
                                class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                                @foreach ($assignableRoles as $roleName)
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <button type="submit" id="btn-send-invite"
                            class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                            {{ __('travel.team.send_invite') }}
                        </button>
                    </div>
                </form>
            </div>
