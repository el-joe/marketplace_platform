...
<div class="p-6 space-y-4">
    ...
    <form method="GET" class="flex flex-wrap gap-3">
        ...
        <select name="country_id" class="rounded-lg border border-gray-300 px-3 py-2 text-sm">
            ...
            <option value="{{ $c->id }}" {{ request('country_id') == $c->id ? 'selected' : '' }}>{{ $c->name_en }}</option>
            @endforeach
        </select>
        <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm">Filter</button>
        <a href="{{ route('admin.travel.agencies.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm">Reset</a>
    </form>
    ...
    <div id="agencies-table-wrap" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div id="agencies-pagination" class="px-4 py-3 border-t border-gray-100 flex items-center gap-2 text-sm text-gray-500"></div>
    </div>
</div>
    ...
    tbody.innerHTML = json.data.length === 0
        ...
        : json.data.map(a => `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                    <a href="/admin/travel/agencies/${a.id}" class="text-primary-600 text-xs font-medium hover:underline">View</a>
                </td>
            </tr>`).join('');
