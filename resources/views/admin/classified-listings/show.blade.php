...
@section('title', 'Listing: ' . $listing->listing_number)
    ...
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                <h2 class="font-semibold text-gray-900">Listing Details</h2>
                <dl class="grid grid-cols-2 gap-3 text-sm">
                    <div><dt class="text-gray-500">Title (EN)</dt><dd class="font-medium">{{ $listing->title_en }}</dd></div>
                    <div><dt class="text-gray-500">Title (AR)</dt><dd class="font-medium" dir="rtl">{{ $listing->title_ar }}</dd></div>
                    <div><dt class="text-gray-500">Category</dt><dd>{{ $listing->classifiedCategory?->name_en }}</dd></div>
                    <div><dt class="text-gray-500">Purpose</dt><dd>{{ ucfirst($listing->listing_purpose) }}</dd></div>
                    <div><dt class="text-gray-500">Price</dt><dd class="font-bold text-gray-900">{{ $listing->price_formatted }}</dd></div>
                    <div><dt class="text-gray-500">Negotiable</dt><dd>{{ $listing->price_negotiable ? 'Yes' : 'No' }}</dd></div>
                    <div><dt class="text-gray-500">Country</dt><dd>{{ $listing->country?->name_en }}</dd></div>
                    <div><dt class="text-gray-500">City</dt><dd>{{ $listing->city?->name_en ?? '—' }}</dd></div>
                    <div><dt class="text-gray-500">Views</dt><dd>{{ number_format($listing->views_count) }}</dd></div>
                    <div><dt class="text-gray-500">Expires</dt><dd>{{ $listing->expires_at?->format('Y-m-d') ?? '—' }}</dd></div>
                </dl>

                @if(!empty($listing->attributes))
                <div>
                    <h3 class="text-sm font-semibold text-gray-700 mb-2">Attributes</h3>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">{{ $listing->description_en }}</p>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-900 mb-3">Images ({{ $listing->images->count() }})</h2>
                <div class="grid grid-cols-4 gap-2">
                    ...
                    <div class="relative aspect-square rounded-lg overflow-hidden border border-gray-100">
                        ...
                        @if($img->is_primary)
                        <span class="absolute bottom-1 start-1 bg-primary-600 text-white text-[10px] px-1 rounded">Primary</span>
                        @endif
                    </div>
                    @endforeach
                </div>
            ...
            {{-- Attachments --}}
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-900 mb-3">Attachments</h2>
                ...
                <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div>
                        ...
                        <a href="{{ \Illuminate\Support\Facades\Storage::url($att->file_path) }}"
                           target="_blank" class="text-xs text-primary-600 hover:underline">View file</a>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium
                        ...
                        <button onclick="verifyAttachment('{{ $att->id }}', 'verify')"
                                class="text-xs text-emerald-600 font-medium hover:underline">Verify</button>
                        @endif
                        @if($att->status !== 'rejected')
                        <button onclick="verifyAttachment('{{ $att->id }}', 'reject')"
                                class="text-xs text-red-500 font-medium hover:underline">Reject</button>
                        @endif
                    </div>
                </div>
                @empty
            ...
            {{-- Inquiries --}}
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-900 mb-3">Inquiries ({{ $listing->inquiries->count() }})</h2>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4">
                <h3 class="font-semibold text-gray-900 mb-3 text-sm">Contract</h3>
                @if($listing->contract_accepted_at)
                <div class="space-y-1.5 text-xs text-gray-600">
                    <p><span class="font-medium">Template:</span> {{ $listing->contractTemplate?->name }}</p>
                    <p><span class="font-medium">Signed:</span> {{ $listing->contract_accepted_at->format('Y-m-d H:i') }}</p>
                    @php $sig = json_decode($listing->contract_signature_data, true); @endphp
                    @if($sig)
                    <p><span class="font-medium">Name:</span> {{ $sig['name'] }}</p>
                    <p><span class="font-medium">IP:</span> {{ $sig['ip'] }}</p>
                    @endif
                </div>
                @else
                <p class="text-xs text-gray-400">Not signed yet.</p>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4">
                <h3 class="font-semibold text-gray-900 mb-2 text-sm">Location</h3>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4">
                <h3 class="font-semibold text-gray-900 mb-2 text-sm">Sketch / Floor Plan</h3>
                <a href="{{ \Illuminate\Support\Facades\Storage::url($listing->sketch_file_path) }}"
                   ...
                   class="text-xs text-primary-600 hover:underline flex items-center gap-1">
                    <x-heroicon name="document" class="w-4 h-4" /> View Sketch
                </a>
            </div>
            @endif
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <h3 class="font-semibold text-gray-900 mb-2 text-sm">QR Code</h3>
            ...
            {{-- Marketers --}}
            <div class="bg-white rounded-xl border border-gray-200 p-4">
                <h3 class="font-semibold text-gray-900 mb-2 text-sm">Marketers</h3>
                @forelse($listing->listingMarketers as $lm)
                <div class="text-xs text-gray-600 py-1 border-b border-gray-50 last:border-0">
                    <p class="font-medium">{{ $lm->marketer?->name ?? 'Open to All' }}</p>
                    <p>{{ $lm->commission_value }}{{ $lm->commission_type === 'percentage' ? '%' : ' fixed' }}</p>
                </div>
                @empty
                <p class="text-xs text-gray-400">No marketers assigned.</p>
                @endforelse
            </div>
...
<div id="reject-modal" class="hidden fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
        <h3 class="font-bold text-gray-900">Reject Listing</h3>
