...
@section('title', __('admin.vendors.application_queue'))

@section('content')

    <div class="mb-6 flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.vendors.application_queue') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('admin.vendors.pending_applications_count', ['count' => $vendors->total()]) }}
            </p>
        </div>
        <a href="{{ route('admin.vendors.index') }}" class="text-sm text-gray-500 hover:text-gray-700">{{ __('admin.vendors.all_vendors') }}</a>
    </div>

    @if($vendors->isEmpty())
        <x-empty-state title="{{ __('admin.vendors.no_pending_applications') }}" description="{{ __('admin.vendors.all_reviewed') }}" icon="inbox" />
    @else
                ...
                <div
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <x-badge :color="$waitColor" class="flex-shrink-0 text-xs">
                            {{ $daysWaiting }}{{ __('admin.vendors.days_suffix') }}
                        </x-badge>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                        <div>
                            <span class="text-gray-400">{{ __('common.email') }}</span>
                            <div class="text-gray-800 truncate">{{ $vendor->email }}</div>
                        </div>
                        <div>
                            <span class="text-gray-400">{{ __('admin.vendors.country_label') }}</span>
                            <div class="text-gray-800">{{ $vendor->country?->name_en ?? '—' }}</div>
                        </div>
                        <div>
                            <span class="text-gray-400">{{ __('admin.vendors.applied_label') }}</span>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1 text-xs">
                            <span class="text-gray-500">{{ __('admin.vendors.documents_verified') }}</span>
                            <span
                    ...
                    <div
                        ...
                        {{ __('admin.vendors.waiting_days', ['days' => $daysWaiting]) }}
                        @if($daysWaiting >= 7)
                            — <strong>{{ __('admin.vendors.action_required') }}</strong>
                        @endif
                    </div>
    ...
    {{-- Quick-reject modal --}}
    <x-modal id="quick-reject-modal" title="{{ __('admin.vendors.reject_application') }}" size="sm">
        <form id="quick-reject-form">
            @csrf
            <input type="hidden" id="qr-vendor-id">
            <p class="text-sm text-gray-600 mb-3">{{ \Illuminate\Support\Str::before(__('admin.vendors.rejecting_notice'), ':name') }}<strong id="qr-vendor-name"></strong>{{ \Illuminate\Support\Str::after(__('admin.vendors.rejecting_notice'), ':name') }}</p>
            <div>
                ...
                <textarea name="rejection_reason" class="form-input w-full resize-none" rows="3"
                    placeholder="{{ __('admin.vendors.quick_reject_reason_placeholder') }}" required></textarea>
            </div>
            <x-slot name="footer">
                <button type="button" data-modal-close class="btn btn-ghost btn-sm">{{ __('admin.vendors.cancel') }}</button>
                <button type="submit" form="quick-reject-form" class="btn btn-danger btn-sm">{{ __('admin.vendors.confirm_rejection') }}</button>
            </x-slot>
        </form>
    </x-modal>
