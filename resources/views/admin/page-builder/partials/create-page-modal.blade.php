...
<x-modal id="create-page-modal" title="Create new page" size="md">
    <form id="create-page-form" class="px-6 py-4 space-y-4">
        ...
        <x-form.select name="country_id" label="Country" required>
            <option value="">— Select —</option>
            @isset($countries)
                @foreach($countries as $c)
                    <option value="{{ $c->id }}">{{ $c->name_en }} ({{ $c->site_code }})</option>
                @endforeach
            @endisset
        </x-form.select>
        ...
        <x-form.input name="reference_id" label="Reference ID (optional)"
                      placeholder="Category / Brand / Seller UUID"
