...
<form data-config-form data-block-id="{{ $block?->id }}">
    ...
    <div class="grid grid-cols-2 gap-3 mt-3">
        <x-form.input name="category_id" label="Category ID" :value="$config['category_id'] ?? ''" />
        <x-form.input name="flash_sale_id" label="Flash sale ID" :value="$config['flash_sale_id'] ?? ''" />
    </div>
