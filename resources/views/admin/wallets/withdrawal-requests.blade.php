...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Review and process withdrawal requests</p>
        </div>
    </div>
    ...
    <div class="flex gap-2">
        @foreach(['', 'pending', 'approved', 'processed', 'rejected'] as $s)
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-medium text-gray-600">Bank</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600">IBAN</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($requests as $wr)
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right font-semibold text-gray-900">{{ number_format($wr->amount_cents / 100, 2) }} {{ $wr->currency }}</td>
                            ...
                            <div class="flex justify-end gap-2">
                                @if($wr->status === 'pending')
                                    <form method="POST" action="{{ route('admin.wallets.withdrawals.approve', $wr) }}">
                                        @csrf @method('PATCH')
                                        <button class="text-xs px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700">Approve</button>
                                    </form>
                                    <button onclick="showRejectModal('{{ $wr->id }}')" class="text-xs px-3 py-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200">Reject</button>
                                @elseif($wr->status === 'approved')
                                    <form method="POST" action="{{ route('admin.wallets.withdrawals.processed', $wr) }}">
                                        @csrf @method('PATCH')
                                        <button class="text-xs px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Mark Processed</button>
                                    </form>
                                @endif
                            </div>
                        </td>
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-10 text-center text-gray-400">No withdrawal requests found.</td></tr>
                @endforelse
            </tbody>
        </table>
...
<div id="reject-modal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form id="reject-form" method="POST">
            @csrf @method('PATCH')
            <textarea name="reason" required rows="3" placeholder="Rejection reason..." class="w-full form-textarea text-sm rounded-lg border-gray-300 mb-4"></textarea>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('reject-modal').classList.add('hidden')" class="px-4 py-2 text-sm text-gray-600">Cancel</button>
                <button type="submit" class="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700">Reject</button>
            </div>
        </form>
    </div>
</div>
