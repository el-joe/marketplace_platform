...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
    ...
    <x-stat-card title="Active" :value="number_format($stats['active'])" icon="check-circle" iconBg="bg-success-100 text-success-600" />
    <x-stat-card title="Suspended" :value="number_format($stats['suspended'])" icon="pause-circle" iconBg="bg-warning-100 text-warning-600" />
    <x-stat-card title="Banned" :value="number_format($stats['banned'])" icon="x-circle" iconBg="bg-danger-100 text-danger-600" />
    <x-stat-card title="New This Week" :value="number_format($stats['new_this_week'])" icon="user-plus" iconBg="bg-primary-100 text-primary-600" />
</div>
...
{{-- ─── Bulk action bar ─────────────────────────────────────────────────────── --}}
<div id="bulk-bar" class="hidden mb-4 flex items-center gap-3 rounded-lg bg-primary-50 border border-primary-200 px-4 py-2">
...
<x-modal id="ban-modal" title="Ban Customer" size="sm" :persistent="true">
    <p class="text-sm text-gray-600 mb-3">You are about to <strong class="text-danger-700">permanently ban</strong> <strong id="ban-customer-name"></strong>. This will revoke all active sessions.</p>
    <div class="mb-3">
        <label class="block text-sm font-medium text-gray-700 mb-1">Reason <span class="text-danger-500">*</span></label>
        <textarea id="ban-reason" class="form-input w-full resize-none" rows="3" placeholder="Enter reason…"></textarea>
    </div>
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Type <code class="bg-gray-100 px-1 rounded">CONFIRM</code> to proceed</label>
