...
<div class="p-6 max-w-2xl mx-auto space-y-4">
    <h1 class="text-xl font-bold text-gray-900">Edit: {{ $category->name_en }}</h1>

    <form method="POST" action="{{ route('admin.classifieds.categories.update', $category) }}"
          class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        @csrf @method('PUT')
        @include('admin.classified-categories._form', ['category' => $category])
