...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <p class="text-xs text-gray-400 mt-1">المبيعات المكتملة في الفترة</p>
        </div>

        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <p class="text-xs text-gray-500 mb-1">إجمالي المرتجعات</p>
            <p class="text-2xl font-bold text-red-500">
                {{ number_format($totalRefunds / 100, 2) }}
                <span class="text-sm font-normal text-gray-400">{{ $currency }}</span>
            </p>
            <p class="text-xs text-gray-400 mt-1">مبالغ مخصومة من أرباحك</p>
        </div>

        <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <p class="text-xs text-gray-500 mb-1">إجمالي المحوّل</p>
            <p class="text-2xl font-bold text-blue-600">
                {{ number_format($totalPaidOut / 100, 2) }}
                <span class="text-sm font-normal text-gray-400">{{ $currency }}</span>
            </p>
            <p class="text-xs text-gray-400 mt-1">دفعات مكتملة في الفترة</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-gray-100">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <h2 class="font-semibold text-gray-800">سجل المعاملات</h2>
                <span class="text-xs text-gray-400">{{ $transactions->total() }} معاملة</span>
            </div>
            ...
            <form id="filter-form" method="GET" action="{{ route('partner.finance.transactions') }}"
                ...
                <div class="flex-1 min-w-48">
                    <label class="block text-xs text-gray-500 mb-1">الفترة الزمنية</label>
                ...
                <div>
                    ...
                    <select name="type" onchange="this.form.submit()"
                        ...
                        <option value="sales"   @selected($type === 'sales')>المبيعات</option>
                        <option value="refunds" @selected($type === 'refunds')>المرتجعات</option>
                        <option value="payouts" @selected($type === 'payouts')>التحويلات</option>
                    </select>
                </div>
            </form>
        </div>
        ...
        @if($transactions->isEmpty())
            <div class="py-16 text-center">
                ...
                <h3 class="font-semibold text-gray-800 mb-1">لا توجد معاملات في هذه الفترة</h3>
                <p class="text-sm text-gray-400">حاول تعديل نطاق التاريخ أو الفلتر.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr class="text-xs text-gray-500 uppercase">
                            <th class="text-right py-3 px-5 font-medium">التاريخ</th>
                            <th class="py-3 px-4 text-center font-medium">النوع</th>
                            <th class="text-right py-3 px-4 font-medium">المرجع</th>
                            <th class="text-right py-3 px-4 font-medium">الوصف</th>
                            <th class="py-3 px-4 text-left font-medium">المبلغ</th>
                        ...
                        @foreach($transactions as $index => $txn)
                            @php
                                ...
                                $typeBadge = match($txn['type']) {
                                    'sale'   => ['bg-green-100 text-green-700', 'مبيعات'],
                                    'refund' => ['bg-red-100 text-red-700',   'مرتجع'],
                                    'payout' => ['bg-blue-100 text-blue-700', 'تحويل'],
                                ...
                                <td class="py-3 px-4 relative">
                                    @if($isSale)
                                        ...
                                        <div data-popover="{{ $popId }}"
                                             ...
                                             style="top: 100%">
                                            <p class="font-semibold text-gray-800 mb-3">تفاصيل الربح</p>
                                            <div class="space-y-2">
                                                ...
                                                <div class="flex justify-between">
                                                    <span class="text-gray-500">عمولة المنصة</span>
                                                    <span class="text-red-500">
                                                        − {{ number_format(($txn['commission'] ?? 0) / 100, 2) }} {{ $currency }}
                                                    </span>
                                                </div>
                                                <div class="flex justify-between border-t border-gray-100 pt-2">
                                                    <span class="font-semibold text-gray-700">صافي الربح</span>
                                                ...
                                                <div class="border-t border-gray-100 pt-2">
                                                    @if($txn['payout_number'])
                                                        <p class="text-gray-500">حالة التسوية:</p>
                                                        <a href="{{ route('partner.payouts.show', $txn['payout_number']) }}"
                                                           class="text-blue-600 hover:underline font-medium">
                                                            مُدرج في {{ $txn['payout_number'] }} →
                                                        </a>
                                                    @else
                                                        <p class="text-gray-400">لم تتم التسوية بعد</p>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
