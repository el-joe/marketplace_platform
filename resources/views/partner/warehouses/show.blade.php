...
@section('content')
        ...
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                @if($warehouse->owner_vendor_id)
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">Seller-owned</span>
                @else
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-700">Platform FBN</span>
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-700">Read-only</span>
                @endif
            ...
            <p class="text-sm text-gray-500">{{ $warehouse->country->name_en }}</p>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            ...
            <p class="text-xs text-gray-500 mt-0.5">Total SKUs</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p class="text-2xl font-bold text-green-700">{{ number_format($usage['total_units']) }}</p>
            <p class="text-xs text-gray-500 mt-0.5">Total units</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            ...
            <p class="text-xs text-gray-500 mt-0.5">Out of stock</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p class="text-2xl font-bold text-red-500">{{ number_format($usage['damaged_units']) }}</p>
            <p class="text-xs text-gray-500 mt-0.5">Damaged</p>
        </div>
    </div>
    ...
    @if($warehouse->total_capacity_m3)
        ...
        <div class="bg-white rounded-2xl border p-4 mb-6">
            <div class="flex justify-between text-sm mb-2">
                <span class="font-medium text-gray-700">Storage capacity</span>
    ...
    @unless($warehouse->owner_vendor_id)
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-medium text-amber-800">Read-only view</p>
                <p class="text-xs text-amber-700 mt-0.5">This is a platform FBN warehouse. Stock adjustments are managed by our warehouse team. Contact support if you see discrepancies.</p>
            </div>
        </div>
    @endunless
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-3 border-b bg-gray-50 flex items-center gap-3 flex-wrap">
            ...
            @if($warehouse->owner_vendor_id)
                <span class="text-xs text-gray-400 ml-auto">Click ± on any row to adjust stock</span>
            @else
                <span class="text-xs text-amber-600 ml-auto font-medium">🔒 Read-only — contact support to adjust platform warehouse stock</span>
            @endif
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b text-xs text-gray-500 uppercase">
                    <tr>
                        ...
                        <th class="px-4 py-2.5 text-right">Reserved</th>
                        <th class="px-4 py-2.5 text-right font-bold text-gray-700">Available</th>
                        <th class="px-4 py-2.5 text-right text-blue-600">Inbound</th>
                        <th class="px-4 py-2.5 text-right text-red-500">Damaged</th>
    ...
    @if($warehouse->owner_vendor_id)
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <select id="adjust-type-select"
                            ...
                            <option value="inbound">Received new stock</option>
                    ...
                    <div>
                        ...
                        <div class="mt-2 p-3 bg-primary-50 rounded-xl flex justify-between">
                            <span class="text-sm text-primary-700">New on-hand:</span>
        ...
        <div id="edit-warehouse-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4">
                <div class="px-6 py-4 border-b flex items-center justify-between">
                    <h3 class="font-semibold text-gray-900">Edit Warehouse</h3>
