...
@php
    ...
    $invStatusMap = [
        ...
        'accepted' => ['label' => 'قبل',         'cls' => 'bg-emerald-100 text-emerald-700'],
        'declined' => ['label' => 'رفض',          'cls' => 'bg-red-100 text-red-700'],
        'expired'  => ['label' => 'منتهية',       'cls' => 'bg-gray-100 text-gray-500'],
        'revoked'  => ['label' => 'مسحوبة',       'cls' => 'bg-gray-100 text-gray-500'],
    ];
@endphp
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
                <div class="px-6 py-4">
                    <h2 class="text-base font-semibold text-gray-900">تفاصيل العرض</h2>
                </div>
                <div class="px-6 py-5 grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                    <div>
                        <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">نوع الحملة</p>
                        <p class="font-medium text-gray-900">{{ $typeMap[$offer->campaign_type] ?? $offer->campaign_type }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">العمولة المقدّمة</p>
                    ...
                    <div>
                        <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">فترة الحملة</p>
                        <p class="font-medium text-gray-900">
                    ...
                    @if ($offer->invitation_deadline)
                        <div>
                            <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">آخر موعد للقبول</p>
                            <p class="font-medium text-gray-900">{{ $offer->invitation_deadline->format('d M Y') }}</p>
                        </div>
                    @endif
                    @if ($offer->budget_per_marketer_cents)
                        <div>
                            <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">ميزانية لكل مسوّق</p>
                            <p class="font-medium text-gray-900">{{ number_format($offer->budget_per_marketer_cents / 100, 2) }} ر.س</p>
                        </div>
                    @endif
                    @if ($offer->total_budget_cents)
                        <div>
                            <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">إجمالي الميزانية</p>
                            <p class="font-medium text-gray-900">{{ number_format($offer->total_budget_cents / 100, 2) }} ر.س</p>
                        </div>
                    @endif
                    <div>
                        <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">نموذج الإسناد</p>
                        <p class="font-medium text-gray-900">{{ ucfirst(str_replace('_', ' ', $offer->attribution_model)) }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">مشاركة واتساب</p>
                        <p class="font-medium text-gray-900">{{ $offer->whatsapp_sharing_enabled ? 'مفعّل' : 'معطّل' }}</p>
                    </div>
                </div>
                @if ($offer->description)
                    <div class="px-6 py-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">الوصف</p>
                        <p class="text-sm text-gray-700 whitespace-pre-line">{{ $offer->description }}</p>
                    </div>
                @endif
                @if ($offer->requirements)
                    <div class="px-6 py-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">المتطلبات</p>
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm px-6 py-5">
                <h2 class="text-base font-semibold text-gray-900 mb-4">مسار الحالة</h2>
                <div class="flex items-center gap-0">
                    @foreach ([
                        ['status' => 'draft',         'label' => 'مسودة'],
                        ['status' => 'pending_admin', 'label' => 'مراجعة الإدارة'],
                ...
                @if ($offer->status === 'rejected' && $offer->rejection_reason)
                    <div class="mt-4 rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
                        <strong>سبب الرفض:</strong> {{ $offer->rejection_reason }}
                    </div>
                @endif
            </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
                <div class="px-6 py-4 flex items-center justify-between">
                    <h2 class="text-base font-semibold text-gray-900">منتجات الحملة</h2>
                    <span class="text-xs text-gray-500">{{ $offer->products->count() }} منتج</span>
                </div>
                <div class="px-6 py-4">
                                ...
                                <div class="flex items-center gap-3 rounded-lg border border-gray-100 p-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        @if ($product->commission_override !== null)
                                            <p class="text-xs text-indigo-600 mt-0.5">عمولة مخصصة: {{ $product->commission_override }}%</p>
                                        @else
                                            <p class="text-xs text-gray-400 mt-0.5">عمولة الحملة: {{ $offer->offered_commission_rate }}%</p>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
        ...
        <div class="space-y-5">
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm px-6 py-4">
                <p class="text-sm text-gray-600 font-medium mb-3">إحصائيات الدعوات</p>
                <div class="flex items-center gap-4 text-sm">
                    <span class="font-bold text-gray-900">{{ $invitationStats['invited'] }}</span><span class="text-gray-500">مدعو</span>
                    <span class="text-gray-300">·</span>
                    <span class="font-bold text-emerald-600">{{ $invitationStats['accepted'] }}</span><span class="text-gray-500">قبلوا</span>
                    <span class="text-gray-300">·</span>
                    <span class="font-bold text-red-500">{{ $invitationStats['declined'] }}</span><span class="text-gray-500">رفضوا/انتهت</span>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
                <div class="px-5 py-4">
                    <h2 class="text-base font-semibold text-gray-900">دعوة مسوّقين</h2>
                </div>
                <div class="px-5 py-4 space-y-3">
                    ...
                    <div class="flex gap-2">
                        <select id="marketer-type-filter"
                            ...
                            <option value="celebrity">مشهور</option>
                            <option value="affiliate">أفلييت</option>
                            <option value="brand_ambassador">سفير براند</option>
                        </select>
                        <input type="text" id="marketer-niche-filter" placeholder="المجال…"
                               class="flex-1 rounded-lg border border-gray-300 px-2 py-2 text-xs focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500">
                    </div>
                    ...
                    <div class="relative">
                        <input type="text" id="marketer-search"
                               placeholder="ابحث عن مسوّق باسمه أو مجاله…"
                    ...
                    <div id="invite-queue" class="hidden">
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-xs font-medium text-gray-600">قائمة الدعوة (<span id="invite-queue-count">0</span>)</p>
                            <button id="btn-clear-queue" type="button" class="text-xs text-red-500 hover:underline">مسح الكل</button>
                        </div>
                        <div id="invite-queue-list" class="space-y-1 max-h-40 overflow-y-auto"></div>
                        <div class="mt-3">
                            <textarea id="vendor-note" rows="2" placeholder="ملاحظة للمسوّقين (اختياري)…"
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm divide-y divide-gray-100">
                <div class="px-5 py-4">
                    <h2 class="text-sm font-semibold text-gray-900">المسوّقون المدعوون</h2>
                </div>
                                ...
                                <div class="mt-1 flex items-center gap-3 flex-wrap">
                                    @if ($inv->status === 'accepted' && $inv->resulting_campaign_id)
                                        <a href="{{ route('partner.marketer-campaigns.show', $inv->resulting_campaign_id) }}"
                                           class="text-xs text-primary-600 hover:underline">عرض الحملة ←</a>
                                    @endif
