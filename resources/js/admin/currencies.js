...
$(function () {
    $(document).on('click', '#btn-dispatch-rate-update', function () {
        ...
        if (!confirm('Queue an exchange rate sync from the API now?')) return;
        ...
        $.ajax({
            ...
            .fail(function (xhr) {
                window.Toast && window.Toast.error(xhr.responseJSON?.message || 'Dispatch failed.');
            })
            .always(function () {
