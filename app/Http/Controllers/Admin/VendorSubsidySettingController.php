...
class VendorSubsidySettingController extends Controller
{
    ...
    public function index(Request $request, Vendor $vendor): View|JsonResponse
    {
        if (!$request->has('draw')) {
            $countries = Country::orderBy('name_en')->get(['id', 'name_en', 'currency_code']);

            return view('admin.vendors.subsidy-settings', compact('vendor', 'countries'));
        }
    ...
    public function store(Request $request, Vendor $vendor): JsonResponse
    {
        ...
        $this->assertNoOverlap($vendor->id, $validated['country_id'], $validated['effective_from'], $validated['effective_until'] ?? null);

        $setting = $vendor->subsidySettings()->create($validated);
    ...
    public function exceptionalZones(Request $request, Vendor $vendor): View|JsonResponse
    {
        if (!$request->has('draw')) {
            return view('admin.vendors.exceptional-zones', compact('vendor'));
        }
    ...
    private function assertNoOverlap(
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'effective_from' => 'This date range overlaps with an existing subsidy setting for this vendor and country.',
            ]);
        }
    }
}
