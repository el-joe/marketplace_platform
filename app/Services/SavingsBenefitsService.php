...
class SavingsBenefitsService
{
    ...
    {
        $methods = Cache::remember("benefits:bnpl:{$countryId}", 300, fn () =>
        ...
        return $methods
            ->filter(fn (CountryPaymentMethod $m) =>
                (!$m->min_order || $m->min_order <= $orderTotal)
                && (!$m->max_order || $m->max_order >= $orderTotal))
            ->map(function (CountryPaymentMethod $m) use ($orderTotal, $currency) {
    ...
    {
        $methods = Cache::remember("benefits:bank:{$countryId}", 300, fn () =>
            CountryPaymentMethod::where('country_id', $countryId)
        ...
        return $methods->map(fn (CountryPaymentMethod $m) => [
    ...
    {
        $offers = Cache::remember("benefits:card_offers:{$countryId}", 300, fn () =>
        ...
        return $offers
            ->filter(fn (CartCardOffer $offer) =>
                (!$offer->valid_from || $offer->valid_from->lte($now))
                && (!$offer->valid_until || $offer->valid_until->gte($now))
                && (!$offer->min_order_amount || $offer->min_order_amount <= $orderTotal))
            ->map(function (CartCardOffer $offer) use ($orderTotal) {
