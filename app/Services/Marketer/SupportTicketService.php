<?php

namespace App\Services\Marketer;

use App\Jobs\NotifyAdminNewTicketJob;
use App\Models\Marketer;
use App\Models\SupportTicket;
use App\Models\TicketMessage;
use Illuminate\Contracts\Pagination\LengthAwarePaginator;
use Illuminate\Support\Str;
use Illuminate\Validation\ValidationException;

class SupportTicketService
{
    public function list(Marketer $marketer): LengthAwarePaginator
    {
        return SupportTicket::where('requester_user_id', $marketer->id)
            ->where('requester_role', 'marketer')
            ->orderByDesc('created_at')
            ->paginate(20);
    }

    public function store(Marketer $marketer, array $data): SupportTicket
    {
        $ticket = SupportTicket::create([
            'ticket_number'      => $this->generateTicketNumber(),
            'requester_user_id'  => $marketer->id,
            'requester_role'     => 'marketer',
            'category'           => $data['category'],
            'priority'           => $data['priority'] ?? 'normal',
            'status'             => 'open',
            'subject'            => $data['subject'],
            'description'        => $data['message'],
            'related_campaign_id'=> $data['campaign_id'] ?? null,
        ]);

        $message = $ticket->messages()->create([
            'sender_type'      => Marketer::class,
            'sender_id'        => $marketer->id,
            'message'          => $data['message'],
            'is_internal_note' => false,
        ]);

        if (!empty($data['attachment'])) {
            $this->storeAttachment($message, $data['attachment']);
        }

        dispatch(new NotifyAdminNewTicketJob($ticket));

        return $ticket;
    }

    public function findForMarketer(Marketer $marketer, string $ticketNumber): ?SupportTicket
    {
        return SupportTicket::where('ticket_number', $ticketNumber)
            ->where('requester_user_id', $marketer->id)
            ->where('requester_role', 'marketer')
            ->with([
                'messages' => fn ($q) => $q->where('is_internal_note', false)
                    ->with('attachments.files')
                    ->oldest('created_at'),
            ])
            ->first();
    }

    public function addMessage(Marketer $marketer, SupportTicket $ticket, array $data): TicketMessage
    {
        $message = $ticket->messages()->create([
            'sender_type'      => Marketer::class,
            'sender_id'        => $marketer->id,
            'message'          => $data['message'],
            'is_internal_note' => false,
        ]);

        if (!empty($data['attachment'])) {
            $this->storeAttachment($message, $data['attachment']);
        }

        return $message;
    }

    public function rate(Marketer $marketer, SupportTicket $ticket, array $data): SupportTicket
    {
        if (!in_array($ticket->status, ['resolved', 'closed'], true)) {
            throw ValidationException::withMessages([
                'ticket' => ['Only resolved or closed tickets can be rated.'],
            ]);
        }

        $ticket->update([
            'satisfaction_rating'  => $data['satisfaction_rating'],
            'satisfaction_comment' => $data['satisfaction_comment'] ?? null,
        ]);

        return $ticket->fresh();
    }

    private function storeAttachment(TicketMessage $message, $file): void
    {
        $attachment = $message->attachments()->create([
            'ticket_message_id' => $message->id,
        ]);
        $path = $file->store('support-tickets', 'public');
        $attachment->files()->create([
            'path'          => $path,
            'disk'          => 'public',
            'mime_type'     => $file->getMimeType(),
            'size'          => $file->getSize(),
            'original_name' => $file->getClientOriginalName(),
        ]);
    }

    private function generateTicketNumber(): string
    {
        return 'TKT-' . strtoupper(Str::random(8));
    }
}
